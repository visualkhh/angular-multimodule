...
(function (global, factory) {
    ...
    class Toaster {
      ...
      _makeHeader(config) {
        ...
        let eTitle = !title.text ? '' : `<strong class="mr-auto">${title.text}</strong>`;
        let eInfo = !title.info ? '' : `<small>${title.info}</small>`;
        let eClose = !title.close ? '' : ` <button type="button" class="ml-2 mb-1 close ${config.buttonOption.class ? config.buttonOption.class : ''}"  ${config.buttonOption.style ? 'style="' + config.buttonOption.style + '"' : ''} data-dismiss="toast" aria-label="Close">
      ...
      static setDefault(opts) {
        for (let k in opts) Default[k] = opts[k];
      }
